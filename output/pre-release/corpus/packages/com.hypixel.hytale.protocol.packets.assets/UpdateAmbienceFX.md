...
Server-to-client packet (ID 62, compressed). Transmits AmbienceFX asset data to the client on the `Default` channel.

## Fields

- UpdateType type
- int maxId
- Map<Integer, AmbienceFX> ambienceFX
...
Deserializes a `UpdateAmbienceFX` from the given buffer at the specified offset.


public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset)

Returns the total number of bytes consumed by this `UpdateAmbienceFX` starting at the given offset.


public void serialize(@Nonnull ByteBuf buf)

Serializes this `UpdateAmbienceFX` into the given buffer.
