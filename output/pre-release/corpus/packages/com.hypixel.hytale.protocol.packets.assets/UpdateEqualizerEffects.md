...
Server-to-client packet (ID 82, compressed). Transmits EqualizerEffects asset data to the client on the `Default` channel.

## Fields

- UpdateType type
- int maxId
- Map<Integer, EqualizerEffect> effects
...
Deserializes a `UpdateEqualizerEffects` from the given buffer at the specified offset.


public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset)

Returns the total number of bytes consumed by this `UpdateEqualizerEffects` starting at the given offset.


public void serialize(@Nonnull ByteBuf buf)

Serializes this `UpdateEqualizerEffects` into the given buffer.
