...
Empty marker component — zero-size (type ID 24).
...
Deserializes a `PropUpdate` from the given buffer at the specified offset.


public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset)

Returns the total number of bytes consumed by this `PropUpdate` starting at the given offset.


@Override
public int serialize(@Nonnull ByteBuf buf)

Serializes this `PropUpdate` into the given buffer. Returns bytes written.
...
Complete API:
  public static PropUpdate deserialize(ByteBuf buf, int offset)
