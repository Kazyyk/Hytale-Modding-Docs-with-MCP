...
An interaction type that extends `SimpleInteraction` with chain pointers. Part of the interaction/ability system (type ID 39).
...
Deserializes a `ResetCooldownInteraction` from the given buffer at the specified offset.


public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset)

Returns the total number of bytes consumed by this `ResetCooldownInteraction` starting at the given offset.


@Override
public int serialize(@Nonnull ByteBuf buf)

Serializes this `ResetCooldownInteraction` into the given buffer. Returns bytes written.
...
Complete API:
  public static ResetCooldownInteraction deserialize(ByteBuf buf, int offset)
