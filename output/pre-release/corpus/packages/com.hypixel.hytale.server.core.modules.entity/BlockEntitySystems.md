...
Container class for ECS systems that manage block entities -- entities that represent in-world blocks with additional behavior (physics, network tracking, visual updates). All inner systems operate on `EntityStore` and are registered by EntityModule during setup.
...
- EntityModule -- registers all three systems during setup
