...
fqcn: "com.hypixel.hytale.builtin.hytalegenerator.engine.bufferbundle.buffers.VoxelBuffer"
api_surface: false
extends: "Buffer"
implements: []
...
A [Buffer](Buffer.md) that stores 3D voxel data on an 8x8x8 grid. Uses a four-state storage strategy: EMPTY (no data), SINGLE_VALUE (uniform content), ARRAY (heterogeneous content), or REFERENCE (copy-on-write delegation to another VoxelBuffer). The REFERENCE state enables efficient buffer copying in `VoxelBufferView.copyFrom` -- the referencing buffer lazily dereferences and allocates its own array only when a write occurs.

## Inner Types

| Type | Kind | Description |
|---|---|---|
| [VoxelBuffer.ArrayContents](VoxelBuffer.ArrayContents.md) | class | Backing array for 8x8x8 voxel data |
| [VoxelBuffer.State](VoxelBuffer.State.md) | enum | Storage state: EMPTY, SINGLE_VALUE, ARRAY, REFERENCE |
