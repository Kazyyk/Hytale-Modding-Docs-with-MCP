...
| `BreakValidatedBlockState` | class | Custom break validation for blocks. |
| `DestroyableBlockState` | class | Health-based destroyable block state. |
| `ItemContainerBlockState` | class | Block-based item container (chests, barrels). |
| `ItemContainerState` | class | Base item container state with slot management. |
| [LaunchPad](LaunchPad.md) | class | Launch pad velocity configuration. |
| `MarkerBlockState` | class | World marker block (spawn points, waypoints). |
| `PlacedByBlockState` | class | Tracks which player placed a block. |
| [RespawnBlock](RespawnBlock.md) | class | Respawn point block state. |
| `SendableBlockState` | class | Flag for client-synchronized block state. |
