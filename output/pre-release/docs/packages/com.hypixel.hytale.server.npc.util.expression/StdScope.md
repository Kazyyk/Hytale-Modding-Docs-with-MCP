...
fqcn: "com.hypixel.hytale.server.npc.util.expression.StdScope"
api_surface: false
extends: null
implements:
...
Standard implementation of [Scope](Scope.md) backed by a `HashMap<String, Symbol>` symbol table and an optional parent scope for hierarchical lookup. Provides methods to add constants, mutable variables, supplier-backed variables, and functions (both invariant and variant). Symbol lookup walks the parent chain when a name is not found locally.

## Fields

| Modifier | Type | Name |
|---|---|---|
| `protected` | [Scope](Scope.md) | `parent` |
...
- [Scope](Scope.md) -- interface this class implements
- [StdLib](StdLib.md) -- singleton subclass providing built-in functions
- [ValueType](ValueType.md) -- type discriminant used by symbols
