...
Server-to-client packet (ID 58, compressed). Transmits FieldcraftCategories asset data to the client on the `Default` channel.
...
Deserializes a `UpdateFieldcraftCategories` from the given buffer at the specified offset.


public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset)

Returns the total number of bytes consumed by this `UpdateFieldcraftCategories` starting at the given offset.


public void serialize(@Nonnull ByteBuf buf)

Serializes this `UpdateFieldcraftCategories` into the given buffer.
