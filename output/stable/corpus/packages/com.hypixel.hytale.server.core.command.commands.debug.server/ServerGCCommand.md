# ServerGCCommand

Type: class | Package: com.hypixel.hytale.server.core.command.commands.debug.server | Extends: CommandBase

public class ServerGCCommand extends CommandBase

Server command extending `CommandBase`.

## Constructors

- ServerGCCommand() | Creates a new ServerGCCommand instance.

## Methods

- executeSync(@Nonnull CommandContext context) | void | protected method.

Also in this package: ServerCommand, ServerDumpCommand, ServerStatsCommand, ServerStatsCpuCommand, ServerStatsGcCommand, ServerStatsMemoryCommand

Complete API:
  protected void executeSync(CommandContext context)
