---
title: "GreaterThanConditionAsset"
kind: "class"
package: "com.hypixel.hytale.builtin.hytalegenerator.assets.materialproviders.spaceanddepth.conditionassets"
fqcn: "com.hypixel.hytale.builtin.hytalegenerator.assets.materialproviders.spaceanddepth.conditionassets.GreaterThanConditionAsset"
api_surface: false
extends: "ConditionAsset"
implements: []
generator_version: "2.0.0"
generated_at: "2026-03-21T00:00:00Z"
tags: 
  - "worldgen"
---

**Package:** `com.hypixel.hytale.builtin.hytalegenerator.assets.materialproviders.spaceanddepth.conditionassets`

```java
public class GreaterThanConditionAsset extends ConditionAsset
```

Concrete implementation extending `ConditionAsset`.

## Fields

| Field | Type | Description |
|---|---|---|
| `parameter` | `ConditionParameter` | ConditionParameter field. |
| `threshold` | `int` | int field. |

## Methods

| Method | Return Type | Description |
|---|---|---|
| `build()` | `SpaceAndDepthMaterialProvider.Condition` | public method. |
