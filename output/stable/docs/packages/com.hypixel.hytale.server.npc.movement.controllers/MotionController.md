...
fqcn: "com.hypixel.hytale.server.npc.movement.controllers.MotionController"
api_surface: false
...
Root interface for all NPC motion controllers. Defines the contract for steering, movement probing, collision queries, velocity estimation, position validation, and movement state management. The three concrete implementations are [MotionControllerWalk](MotionControllerWalk.md), [MotionControllerFly](MotionControllerFly.md), and [MotionControllerDive](MotionControllerDive.md), all via [MotionControllerBase](MotionControllerBase.md).
...
| `double probeMove(Ref<EntityStore>, ProbeMoveData, ComponentAccessor<EntityStore>)` | Probes movement using pre-configured [ProbeMoveData](ProbeMoveData.md) |
