...
fqcn: "com.hypixel.hytale.server.npc.util.expression.ExecutionContext"
api_surface: false
extends: null
implements: []
...
Stack-based virtual machine that executes compiled expression instruction sequences. Maintains an operand stack of `Operand` slots (auto-growing by increments of 8), a current [Scope](Scope.md), and optional combat config / interaction variable context. Provides pre-defined static `Instruction` constants for all arithmetic, logical, bitwise, and comparison operators, plus factory methods for generating PUSH, READ, CALL, and PACK instructions.
...
| `protected` | [Scope](Scope.md) | `scope` |
| `protected` | `Operand[]` | `operandStack` |
| `protected` | `int` | `stackTop` |
| `protected` | [ValueType](ValueType.md) | `lastPushedType` |
...
|---|---|---|
| `public` | [ValueType](ValueType.md) | `execute(@Nonnull List<Instruction> instructions, Scope scope)` |
| `public` | [ValueType](ValueType.md) | `execute(@Nonnull List<Instruction> instructions)` |
| `public` | [ValueType](ValueType.md) | `execute(@Nonnull Instruction[] instructions, Scope scope)` |
| `public` | [ValueType](ValueType.md) | `execute(@Nonnull Instruction[] instructions)` |
...
Mutable value holder on the operand stack. Stores one value of any supported type (`string`, `number`, `bool`, `numberArray`, `stringArray`, `boolArray`) plus a [ValueType](ValueType.md) discriminant. Reused across evaluations to minimize allocation.

## Related Types

- [Expression](Expression.md) -- high-level compile/execute entry point
- [Scope](Scope.md) -- variable/function provider
- [ValueType](ValueType.md) -- type discriminant
