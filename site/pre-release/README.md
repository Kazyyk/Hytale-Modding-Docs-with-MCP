...
The generated markdown in `output/pre-release/docs/` is the source of truth. At build time, a sync script copies those files into Starlight's `src/content/docs/` directory and patches the landing page with Starlight's splash template.

```
output/pre-release/docs/  →  scripts/sync-docs.mjs  →  src/content/docs/  →  astro build  →  dist/
```
...
│   ├── content/docs/         # Synced from output/pre-release/docs/ (gitignored)
