...
const SOURCE_DIR = join(PROJECT_ROOT, "output/pre-release/docs");
const TARGET_DIR = join(PROJECT_ROOT, "site/pre-release/src/content/docs");
...
function patchLandingPage(content) {
  ...
  if (!content.includes("template:")) {
    content = content.replace(
      /^(---\n(?:.*\n)*?title:\s*.*\n)/m,
      "$1template: splash\nhero:\n  tagline: Comprehensive Hytale pre-release source documentation — every type, every method, every system\n  actions:\n    - text: Browse Packages\n      link: /packages/com-hypixel-hytale-server-core-plugin/\n      icon: right-arrow\n    - text: Configure MCP\n      link: /mcp/\n      icon: right-arrow\n      variant: secondary\n"
    );
  }
  return content;
}
